...
export class MasterService {
    ...
    public async masterItems(): Promise<Master[]> {
        const masterRes = await this.masterRepository.find({ relations: ['extraction', 'massSpec', 'receiving', 'screening', 'quality', 'storeRoom', 'rd', 'chemical'] });
        if (!masterRes) {
    ...
    public async masterItem(id: number): Promise<Master> {
        const masterItemRes = await this.masterRepository.findOne(id, { relations: ['extraction', 'massSpec', 'receiving', 'screening', 'quality', 'storeRoom', 'rd', 'chemical'] });
        if(!masterItemRes) {
            throw new NotFoundException();
        };
